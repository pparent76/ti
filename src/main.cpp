...
main(int argc, char **argv) {
    ...
    vector<TiString> titlesPrgrm;
    titlesPrgrm.push_back(TiString("Exec"));
    titlesPrgrm.push_back(TiString("Edit"));
    titlesPrgrm.push_back(TiString("New"));

    vector<TiString> tmpCreateNew;
    tmpCreateNew.push_back(TiString("Create New"));
    vector<vector<TiString> > choicesPgrm;
    choicesPgrm.push_back(programsMenu);
    choicesPgrm.push_back(programsMenu);
    choicesPgrm.push_back(tmpCreateNew);
...
// 		  cout<<":"<<program[i].toStdString()<<endl;
                    ...
                    if (menuPrgrm.getSelectedTitle()==1)
                    {
                ...
                else if(event.key.keysym.sym==SDLK_ESCAPE and mode==6)
                {
		        SDL_EnableKeyRepeat(0,20);
                        mode=1;
			editor.saveProgram();
                ...
                else
                {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        editor.sendKey( event.key.keysym);
                        break;
                    }
                }
                break;
